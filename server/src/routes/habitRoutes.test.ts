...
describe.skip('Habit Routes (/api/habits)', () => { // Skipping this entire suite
  ...
  beforeEach(() => {
    ...
    (admin.auth().verifyIdToken as jest.Mock).mockResolvedValue(mockUserToken);
    // Global mock for serverTimestamp for consistency in habit tests
    (admin.firestore.FieldValue.serverTimestamp as jest.Mock).mockReturnValue(MOCK_DATE_ISO);
  });
  ...
  describe('POST / (Create Habit)', () => {
    it('should create a new binary habit successfully', async () => {
      ...
      (admin.firestore().collection('habits').add as jest.Mock).mockResolvedValue(mockHabitRef);


      const req = new Request('http://localhost/', { // Path relative to the app from habitRoutes.ts
        method: 'POST',
        headers: {
    ...
    it('should create a new quantitative habit successfully', async () => {
        ...
        const req = new Request('http://localhost/', { // Path relative to the app
            method: 'POST',
            headers: {
              'Authorization': 'Bearer test-token',
              'Content-Type': 'application/json'
  ...
  describe('GET / (Fetch Habits)', () => {
    it('should fetch active habits for the user', async () => {
      ...
      (admin.firestore().collection('habits').where as jest.Mock).mockImplementation(mockWhere1Fn);
    ...
    it('should return an empty array if no active habits found', async () => {
        ...
        const mockWhere2Fn = jest.fn(() => ({ get: mockGetFn }));
        (admin.firestore().collection('habits').where as jest.Mock).mockImplementationOnce(() => ({ where: mockWhere2Fn }));
  ...
  describe('DELETE /:habitId (Archive Habit)', () => {
    it('should archive an active habit successfully', async () => {
      ...
      (mockDocRef.get as jest.Mock).mockResolvedValue({ exists: true, data: () => mockHabitData });
      (mockDocRef.update as jest.Mock).mockResolvedValue(undefined);

      const req = new Request(`http://localhost/${habitId}`, { // Path relative to the app
        method: 'DELETE',
        headers: { 'Authorization': 'Bearer test-token' },
      });
    ...
    it('should return 403 if habit does not belong to user', async () => {
      ...
      (admin.firestore().collection('habits').doc(habitId).get as jest.Mock).mockResolvedValue({
        exists: true,
    ...
    it('should return 404 if habit not found', async () => {
      ...
      (admin.firestore().collection('habits').doc(habitId).get as jest.Mock).mockResolvedValue({ exists: false });
      const req = new Request(`http://localhost/${habitId}`, { // Path relative to the app
        method: 'DELETE',
        headers: { 'Authorization': 'Bearer test-token' },
      });
  ...
  describe('POST /:habitId/complete (Log Completion)', () => {
    ...
    it('should add a new completion entry if none exists for today', async () => {
      ...
      const req = new Request(`http://localhost/${habitId}/complete`, { // Path relative to the app
        method: 'POST',
        headers: {
          'Authorization': 'Bearer test-token',
          'Content-Type': 'application/json',
      ...
      expect(res.status).toBe(200);
      const updateCallArgs = (mockDocRef.update as jest.Mock).mock.calls[0][0];
  ...
  describe('PUT /:habitId (Update Habit)', () => {
    ...
    let originalHabitData: FirestoreHabit; // Define type here
    let quantHabitData: FirestoreHabit;   // Define type here
    ...
    it('should update habit fields successfully', async () => {
      ...
      (mockInitialDocRef.get as jest.Mock).mockResolvedValueOnce({ // For initial fetch
        exists: true,
        data: () => originalHabitData, // Uses originalHabitData from beforeEach
      });
      (mockInitialDocRef.update as jest.Mock).mockResolvedValue(undefined);

      const updatedFields = { title: 'New Title', description: 'New Desc' };
      // For fetch after update
      (admin.firestore().collection('habits').doc(habitId).get as jest.Mock).mockResolvedValueOnce({
        exists: true,
    ...
    it('should correctly handle type change from quantitative to binary (delete target fields)', async () => {
      ...
      const mockInitialDocRef = admin.firestore().collection('habits').doc(habitId);
      (mockInitialDocRef.get as jest.Mock).mockResolvedValueOnce({
      ...
      const expectedDataAfterUpdate: Partial<FirestoreHabit> = {
